...
| `dev-db-server.mjs` | A real Postgres for local dev with nothing to install: PGlite (Postgres compiled to WebAssembly) served over TCP, so the app connects exactly as it would to Supabase. Applies the schema on boot. Data lives under `node_modules/.cache` — **not** the project root, where Tailwind's scanner chokes on the binary files and takes the whole app down with a bogus `globals.css` error. | none |
| `portal-e2e.mjs` | Exercise the client portal against the local database and verify **in the database** that each action landed. Flows that need external services (App bridge, Supabase Storage) are reported as not-verifiable rather than failures. | `DATABASE_URL` |
| `dev-db.mjs` | Stand up a working client portal on a **scratch** database, so it can be exercised without waiting for a paying client: `migrate` (applies pending migrations — no `psql` needed), `seed` (demo client + workspace + a thread), `status`, `unseed`. Refuses to run against a DB holding non-demo sessions unless `--force`; demo rows are all `dev-demo*`, so `unseed` can't reach real data. Shortcuts: `npm run db:migrate` / `db:seed` / `db:status`. | `DATABASE_URL` (+ optional `DEV_VIEWER_EMAIL`) |
