...
/**
 * ManageMembershipButton — opens the Stripe Billing Portal so the client can
 * update their payment method, switch plan or cancel (v3 membership billing M2 /
 * Fase 6b). Serves BOTH plans: a membership's monthly and a one-time buyer's
 * yearly hosting are the same Stripe subscription shape, so the same portal
 * manages them — only the label changes (`label` prop).
 *
 ...
 * updated state (fed by the M1 webhook → App → project-status pull).
 */
...
export function ManageMembershipButton({
  ...
  sessionId: string;
  /** "Manage hosting" for a one-time buyer — they never had a membership. */
  label?: string;
}) {
