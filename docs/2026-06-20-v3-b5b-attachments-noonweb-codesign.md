...
>
> ### 🔧 BACKEND DE HOSTING (post-freeze, 2026-06-20) — Vercel Blob → **Supabase Storage**
>
> Al ir a construir se verificó (docs oficiales de Vercel Blob + skill): **el `access:'private'` de Vercel Blob NO ofrece signed URLs de corta vida** — su delivery privado es "stream a través de tu Function vía `get()`" (proxy de bytes). Eso **rompe el mecanismo de acceso co-firmado** (Q-B5b-2: `302` a una URL firmada de corta vida, **sin** proxy de bytes del App). **Supabase Storage `createSignedUrl(path, ttl)` SÍ da URLs firmadas de corta vida** → cumple Q-B5b-2 exactamente, sin proxy. Como el storage es **detalle interno de NoonWeb** (el App solo ve nuestro signed-read → `302`), **el contrato App-facing NO cambia y NO requiere re-co-firma.** Único cambio: el **env** pasa de `BLOB_READ_WRITE_TOKEN` a **`SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`** (sigue siendo NoonWeb-only; Q-B5b-6 intacto). Acceso via la **Storage REST API** (sin dep nueva). Donde abajo este doc dice "Vercel Blob"/"`BLOB_READ_WRITE_TOKEN`", léase **Supabase Storage** / **`SUPABASE_URL`+`SUPABASE_SERVICE_ROLE_KEY`**; todo lo demás (wire, sub-shape, `302` a signed URL, límites, lifecycle) idéntico.
